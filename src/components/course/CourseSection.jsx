...
const DevOpsCourseSection = (props) => {
  const cardStyle = {
...
//   );
  ...
  <ScrollReveal animation="slightRight" duration={1200} delay={300} easing="spring" className=''>
        <h1 className="relative text-3xl md:text-5xl lg:text-7xl mb-6 md:mb-12 leading-tight">
          <span className="block text-red-500 text-lg md:text-2xl mb-2 uppercase">THE ONLY {props.courseName} COURSE THAT</span>
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 lg:gap-16 relative max-w-full">
    {/* Card 1 */}
    {props.courseDescription.map((desc, index) => (
      <ScrollReveal key={index} animation="slideUp" duration={1200} delay={`${(index + 1) * 25}`} easing="spring" className='grid' >
      <div className="bg-black bg-opacity-50 border border-[#1DD9FF] p-6 md:p-8 rounded-lg relative">
        <div className="bg-gradient-to-b from-white to-transparent bg-clip-text text-transparent font-bold text-4xl md:text-8xl h-10 w-10 md:h-20 md:w-20 flex items-center justify-center mb-4">{index + 1}</div>
        <div className="text-xl md:text-2xl font-bold text-white mb-3">{desc.title}</div>
